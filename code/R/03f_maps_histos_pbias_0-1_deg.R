...
results <- read.csv(paste(main.path,"/Results/pbias.txt",sep=""),sep="\t")

fig_width <- 8/2.54 #in inches

#performance color palette -----
palette <- "cork" #color palette pbias
direction <- -1
palette_min <- -50
palette_max <- 50
...
for (v in c("ptq_camels.evap_camels","pt_q_camels.evap_estreams","p_t_estreams_q.evap_estreams",
  ...
  svg(paste(main.path,"/Plots/performances/",v,"_pbias_map.svg",sep=""),
  ...
  points(data$lon[order(abs(data[,v]))],data$lat[order(abs(data[,v]))],
         pch=20,col=data$color[order(abs(data[,v]))],cex=0.6) #plot small pbias first
...
for (v in c("ptq_camels.evap_camels","pt_q_camels.evap_estreams","p_t_estreams_q.evap_estreams",
  ...
  svg(paste(main.path,"/Plots/performances/",v,"_pbias_histos_landscape.svg",sep=""),
      width=fig_width,height=histo_height_landscape)
  par(mfrow=c(4,2),mar=c(0.4,1.4,0.5,0.4))
  for (c in c("all",countries)){
    ...
    axis(2,at=c(which(histo$mids==min(histo$mids))-0.5,which(histo$mids==0)-0.5,which(histo$mids==max(histo$mids))-0.5),
         labels=c(paste("≤",min(histo$mids),sep=""),0,paste("≥",max(histo$mids),sep="")),cex.axis=0.55,tick=FALSE,line=-0.9,las=2)
    ...
    lines(x=c(0,max(histo$counts)),y=c(which(histo$mids==0)-0.5,which(histo$mids==0)-0.5),lty="dotted",lwd=0.5)
...
svg(paste(main.path,"/Plots/performances/pbias_legend.svg",sep=""),
