...
palette <- "lipari" #color palette performance
direction <- -1
palette_min <- 0
palette_max <- 1
...
#performance maps -----
# p_estreams_tq.evap_camels: p estreams,  t and evap camels
# pt_q_camels.evap_camels: p and t estreams, evap camels
# ptq_camels.evap_camels: p, t, evap camels
# p_t_estreams_q.evap_estreams: p camels, t and evap estreams
# pt_q_camels.evap_estreams: p, t, evap estreams
# ptq_camels.evap_estreams: p and t camels, evap estreams

map_width <- fig_width*0.85
map_height <- map_width/0.85 #nice ratio for Europe map

for (v in c("p_estreams_tq.evap_camels","pt_q_camels.evap_camels","ptq_camels.evap_camels",
            "p_t_estreams_q.evap_estreams","pt_q_camels.evap_estreams","ptq_camels.evap_estreams")){
  data <- join(topology,results[,c("id",v)],by="id")
  data$color <- col_fun(data[,v])
  svg(paste(main.path,"/Plots/performances/",v,"_performance_map.svg",sep=""),
  ...
  points(data$lon[order(-data[,v])],data$lat[order(-data[,v])],
         pch=20,col=data$color[order(-data[,v])],cex=0.6) #plot high performances first
...
step <- 0.05 #width of the buckets of the histogram
low <- 0 #minimum of histogram
high <- 1 #maximum of histogram
...
for (v in c("p_estreams_tq.evap_camels","pt_q_camels.evap_camels","ptq_camels.evap_camels",
            "p_t_estreams_q.evap_estreams","pt_q_camels.evap_estreams","ptq_camels.evap_estreams")){
  for (c in countries){
    data_hist <- results[substr(results$id,1,2)==c,v]
  ...
  svg(paste(main.path,"/Plots/performances/",v,"_performance_histos_portrait.svg",sep=""),
  ...
  for (c in countries){
    ...
    axis(2,at=c(which(histo$mids==min(histo$mids))-0.5,which(histo$mids==0.5)-0.5,which(histo$mids==max(histo$mids))-0.5),
         labels=c(paste("≤",min(histo$mids),sep=""),0.5,max(histo$mids)),cex.axis=0.5,tick=FALSE,line=-0.9,las=2)
    ...
    lines(x=c(0,max(histo$counts)),y=c(which(histo$mids==0.5)-0.5,which(histo$mids==0.5)-0.5),lty="dotted",lwd=0.5)
  ...
  svg(paste(main.path,"/Plots/performances/",v,"_performance_histos_landscape.svg",sep=""),
  ...
  for (c in c("all",countries)){
    ...
    axis(2,at=c(which(histo$mids==min(histo$mids))-0.5,which(histo$mids==0.5)-0.5,which(histo$mids==max(histo$mids))-0.5),
         labels=c(paste("≤",min(histo$mids),sep=""),0.5,max(histo$mids)),cex.axis=0.55,tick=FALSE,line=-0.9,las=2)
    lines(x=c(0,max(histo$counts)),y=c(which(histo$mids==min(histo$mids))-0.5,which(histo$mids==min(histo$mids))-0.5),lty="dotted",lwd=0.5)
    lines(x=c(0,max(histo$counts)),y=c(which(histo$mids==0.5)-0.5,which(histo$mids==0.5)-0.5),lty="dotted",lwd=0.5)
    lines(x=c(0,max(histo$counts)),y=c(which(histo$mids==max(histo$mids))-0.5,which(histo$mids==max(histo$mids))-0.5),lty="dotted",lwd=0.5)
    axis(1,at=c(round(sum(histo$counts)/10),max(histo$counts)),
